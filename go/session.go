...
func setSession(username string, w http.ResponseWriter, db *sql.DB) error {
	...
	_, err := db.Exec("UPDATE Users SET CurrentSessionID = ? WHERE Username = ?", newSessionID, username)
	if err != nil {
...
// checkSession checks if the user is already logged in on a different browser (ignore below for now)
func checkSession(username string, db *sql.DB) (bool, error) {
	var currentSessionID string

	// Corrected to use Username
	err := db.QueryRow("SELECT CurrentSessionID FROM Users WHERE Username = ?", username).Scan(&currentSessionID)
	if err != nil {
